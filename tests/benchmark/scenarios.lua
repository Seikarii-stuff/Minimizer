...
function M.coreScaling()
    local results = {}
    for _, n in ipairs({1, 10, 25, 50, 100}) do
        H.resetState()
        H.makeUnits(n)
        ...
        end, { subsystem = "Core/Dispatcher", units = n })
    end
    return results
end
...
function M.decisionClassification()
    ...
    end, { subsystem = "Classification", units = 1 })
    ...
    end, { subsystem = "Decision", units = 1 })
    ...
    for _, n in ipairs({10, 50, 100}) do
        ...
        results[#results + 1] = bench("decision.apply_all." .. n, "ApplyToUnit per active plate", function(batch)
            for _ = 1, batch do A.Dispatcher.ApplyToAll(false) end
        end, { subsystem = "Decision + Snapshot + modules", units = n })
    end
    return results
end
...
function M.threatScaling()
    local results = {}
    for _, n in ipairs({1, 5, 10, 20, 50}) do
        ...
        end, { subsystem = "Threat", units = n, state = "stable/cached" })
        ...
        end, { subsystem = "Threat", units = n, state = "changed/event path" })
    end
    return results
end
...
function M.dispatcherEvents()
    ...
    for _, n in ipairs({10, 50, 100}) do
        ...
        end, { subsystem = "Events + Dispatcher", units = n, workload = "mixed plausible unit events" })
    end
    return results
end
...
function M.castScaling()
    local results = {}
    for _, casters in ipairs({1, 5, 10, 20}) do
        ...
        end, { subsystem = "Cast", casters = casters })
        ...
        end, { subsystem = "Cast + integrated pipeline", casters = casters })
    end
    return results
end
...
function M.absorbScaling()
    local results = {}
    for _, n in ipairs({1, 10, 50, 100}) do
...
function M.haloAndWheel()
    ...
    end, { subsystem = "Halo", path = "cold/create" }, { batch = 20, samples = 7 })
    ...
    end, { subsystem = "Halo", path = "hot/reused" })
    ...
    end, { subsystem = "Wheel", state = "disabled" })

    _G.MinimizerDB.wheelEnabled = true
    A.Wheel:ApplyConfig()
    results[#results + 1] = bench("wheel.active", "Wheel.Update", function(batch)
        for _ = 1, batch do A.Wheel:Update() end
    end, { subsystem = "Wheel", state = "active" })
...
function M.targetFocus()
    ...
    Mocks.units.target.guid = "target-guid"
    Mocks.units.focus.guid = "focus-guid"
...
function M.steadyState()
    ...
    for _, n in ipairs({10, 50, 100}) do
        ...
        results[#results + 1] = bench("steady_state." .. n, "ApplyToAll with unchanged state", function(batch)
            for _ = 1, batch do A.Dispatcher.ApplyToAll(false) end
        end, { subsystem = "Integrated", units = n, workload = "steady-state/no intentional changes" })
    end
    return results
end
...
function M.recycling()
    ...
    for _, n in ipairs({1, 10, 50}) do
            ...
            for cycle = 1, batch do
                ...
                if not Mocks.units[unit] then
                    ...
                    if unit ~= created then
                        Mocks.units[unit] = Mocks.units[created]
                        Mocks.nameplates[unit] = Mocks.nameplates[created]
                        Mocks.units[created] = nil
                        Mocks.nameplates[created] = nil
                    end
                end
                local plate = Mocks.nameplates[unit]
                ...
                Mocks.units[unit] = {
                    name = "Reused " .. unit, health = 100, healthMax = 100,
                    faction = "Horde", isPlayer = false, classification = "normal",
                    threatSituation = 1, guid = "reused-" .. cycle,
                }
        ...
        end, { subsystem = "Lifecycle", units = n, cycles = cycles })
    end
    return results
end
        ...
        results[#results + 1] = bench(w.name, "representative event/update workload", function(batch)
            for i = 1, batch do
                ...
                if w.recycle and i % 17 == 0 then
                    A.Lifecycle.ClearNeverSimplify(unit)
                    A.Dispatcher.ForgetUnit(unit)
                    A.Lifecycle.IncrementGeneration(unit)
                    A.Dispatcher.TrackUnit(unit)
                end
            end
        end, { subsystem = "Integrated", units = w.units, casters = w.casters, threat = w.threat, recycling = w.recycle })
    end
    return results
end
...
function M.all()
    ...
    local groups = {
        { "core", M.coreScaling },
        { "decision", M.decisionClassification },
        { "threat", M.threatScaling },
        { "events", M.dispatcherEvents },
        { "cast", M.castScaling },
        { "absorb", M.absorbScaling },
        { "overlays", M.targetFocus },
        { "halo_wheel", M.haloAndWheel },
        { "pips", M.pips },
        { "steady", M.steadyState },
        { "recycling", M.recycling },
        { "integrated", M.integratedWorkloads },
    }
