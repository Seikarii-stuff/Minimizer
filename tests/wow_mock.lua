...
function _G.GetTime() return Mocks.time end
...
function FrameMixin:RegisterEvent(event)
    local reg = rawget(self, "registeredEvents") or {}
...
function FrameMixin:GetStatusBarColor()
    if self.statusBarColor then return unpack(self.statusBarColor) end
    return 1, 1, 1, 1
end
...
function FrameMixin:GetParent() return rawget(self, "parent") end
function FrameMixin:SetAllHitTestPoints(clickRegion) self.MinimizerHitTestRegion = clickRegion end
...
setmetatable(FrameMixin, {
    __index = function(t, k)
        if type(k) == "string" then
            if k:match("^Create") then return dummyCreateObject
            elseif k:match("^Set") or k:match("^Clear") or k:match("^Play") or k:match("^Stop") or k:match("^Hook") then return dummyMethod
            elseif k:match("^Get") then return function() return 1, 1, 1, 1 end
            elseif k:match("^Is") or k:match("^Can") then return function() return true end
            elseif k:match("^Add") then return dummyMethod end
        end
        return nil
    end
})
...
function _G.CreateFrame(frameType, name, parent, template)
    local frame = { type = frameType, name = name, parent = parent, template = template, shown = true, alpha = 1 }
...
_G.C_Timer = {
    After = function(duration, callback)
        if duration <= 0 then callback() else table.insert(Mocks.timers, { fireTime = Mocks.time + duration, callback = callback }) end
    end,
}
_G.C_Timer.NewTicker = function(interval, callback)
    ...
    local function reschedule()
        ...
        table.insert(Mocks.timers, { fireTime = Mocks.time + interval, callback = function() callback(ticker); reschedule() end })
    end
    reschedule()
...
_G.C_CurveUtil = {
    EvaluateColorValueFromBoolean = function(state, valueIfTrue, valueIfFalse)
        ...
        if type(state) == "table" and state.__minimizerMockSecret then resolved = state.__value end
        if resolved == true then return valueIfTrue else return valueIfFalse end
    end
}
Mocks.playerSpells = { [107574] = true, [1719] = true, [171138] = true, [8122] = true, [642] = true, [118000] = true }
_G.C_SpellBook = {
    IsSpellKnownOrInSpellBook = function(spellID) return Mocks.playerSpells[spellID] == true end,
    IsSpellKnown = function(spellID) return Mocks.playerSpells[spellID] == true end,
}
_G.IsPlayerSpell = function(spellID) return Mocks.playerSpells[spellID] == true end
_G.IsSpellKnown = function(spellID) return Mocks.playerSpells[spellID] == true end
function Mocks.Secret(value) return { __minimizerMockSecret = true, __value = value } end
_G.issecretvalue = function(v) return type(v) == "table" and v.__minimizerMockSecret == true end
function _G.hooksecurefunc(table, funcName, hook)
    if type(table) == "string" then hook = funcName; funcName = table; table = _G end
    local orig = table[funcName]
    table[funcName] = function(...)
        local result = nil
        if orig then result = orig(...) end
        hook(...)
        return result
    end
end
...
_G.C_NamePlate = {
    GetNamePlates = function()
        local nps = {}
        for _, np in pairs(Mocks.nameplates) do table.insert(nps, np) end
        return nps
    end,
    GetNamePlateForUnit = function(unit)
        for _, np in pairs(Mocks.nameplates) do if np.namePlateUnitToken == unit then return np end end
        return nil
    end,
}
_G.C_NamePlateManager = {
    SetNamePlateSimplified = function(unit, simplified)
        ...
        if np then np.simplified = simplified end
    end
}
local function getUnit(unitId) return Mocks.units[unitId] end
function _G.UnitExists(unit) return getUnit(unit) ~= nil end
function _G.UnitName(unit) local u = getUnit(unit); return u and u.name or "Unknown" end
function _G.UnitHealth(unit) local u = getUnit(unit); return u and u.health or 0 end
function _G.UnitHealthMax(unit) local u = getUnit(unit); return u and u.healthMax or 100 end
function _G.UnitLevel(unit) local u = getUnit(unit); return u and u.level or 1 end
function _G.UnitEffectiveLevel(unit) return _G.UnitLevel(unit) end
function _G.UnitClassification(unit)
    ...
    local u = getUnit(unit); return u and u.classification or "normal"
end
function _G.UnitIsLieutenant(unit) local u = getUnit(unit); return u and u.isLieutenant or false end
function _G.UnitCanAttack(unit1, unit2)
    local u1, u2 = getUnit(unit1), getUnit(unit2)
    if not u1 or not u2 then return false end
    return u1.faction ~= u2.faction
end
function _G.UnitAffectingCombat(unit) local u = getUnit(unit); return u and u.inCombat == true or false end
function _G.UnitGroupRolesAssigned(unit) local u = getUnit(unit); return u and (u.role or "NONE") or nil end
function _G.IsInRaid() return Mocks.inRaid == true end
function _G.IsInGroup() return Mocks.inGroup == true end
Mocks.inGroup = true
Mocks.inRaid = false
function _G.UnitIsUnit(unit1, unit2)
    if unit1 == unit2 then return true end
    local u1, u2 = getUnit(unit1), getUnit(unit2)
    return u1 and u2 and u1.guid == u2.guid or false
end
function _G.UnitThreatSituation(source, target)
    local u = getUnit(target)
    if not u then return nil end
    return source == "player" and u.threatSituation or 0
end
function _G.UnitCastingInfo(unit)
    ...
    if u and u.cast then return u.cast.name, nil, nil, u.cast.startTime, u.cast.endTime, nil, u.cast.castID, u.cast.uninterruptible end
    return nil
end
function _G.UnitChannelInfo(unit)
    Mocks.unitChannelInfoCallCounts[unit] = (Mocks.unitChannelInfoCallCounts[unit] or 0) + 1
    local u = getUnit(unit)
    if u and u.channel then return u.channel.name, nil, nil, u.channel.startTime, u.channel.endTime, nil, u.channel.uninterruptible end
    return nil
end
function _G.UnitAura(unit, index, filter)
    ...
    for _, aura in ipairs(u.auras) do
        ...
        if match then
            count = count + 1
            if count == index then return aura.name, aura.icon, aura.count, aura.debuffType, aura.duration, aura.expirationTime, aura.source, aura.isStealable, aura.nameplateShowPersonal, aura.spellId end
        end
    end
    return nil
end
    ...
    ForEachAura = function(unit, filter, maxCount, func, useAuraInstanceID)
        ...
        for _, aura in ipairs(u.auras) do
            ...
            if match then
                ...
                if result == true or (maxCount and count >= maxCount) then break end
            end
        end
    end
}
function _G.UnitIsPlayer(unit) local u = getUnit(unit); return u and u.isPlayer or false end
function _G.UnitIsDead(unit) local u = getUnit(unit); return u and u.dead or false end

function Mocks.CreateTestUnit(unit, data)
    data = data or {}
    data.health = data.health or 100
    data.healthMax = data.healthMax or 100
    Mocks.units[unit] = data
end
function Mocks.CreateTestNameplate(unit)
    local plate = CreateFrame("Frame", "MockNamePlate_" .. unit, UIParent)
    plate.namePlateUnitToken = unit
    plate.UnitFrame = plate
    plate.healthBar = CreateFrame("StatusBar", "MockHealthBar_" .. unit, plate)
    plate.healthBar:SetStatusBarColor(1, 1, 1, 1)
    plate.HealthBar = plate.healthBar
    Mocks.nameplates[unit] = plate
    return plate
end

function Mocks.ResetCooldowns()
    Mocks.cooldowns = {}
end
