...
check(info.name == nil, "GetInfo: does not expose redundant name")

local infoCalls = 0
_G.C_Spell.GetSpellInfo = function(id)
    infoCalls = infoCalls + 1
    return { iconID = "modernIcon" }
end
Spells.InvalidateCache()
local firstInfo = Spells.GetInfo(321)
local secondInfo = Spells.GetInfo(321)
check(infoCalls == 1 and firstInfo.texture == "modernIcon" and secondInfo.texture == "modernIcon",
    "_info_cache: GetSpellInfo is cached and not called twice")

Spells.InvalidateCache()
local refreshedInfo = Spells.GetInfo(321)
check(infoCalls == 2 and refreshedInfo.texture == "modernIcon", "_info_cache: invalidation refreshes GetSpellInfo")
...
_G.C_Spell = { GetBaseSpell = function(id) return id end, GetSpellInfo = function(id) return { iconID = "icon" } end, GetSpellCooldownDuration = function(id) return { IsZero = function() return false end } end, GetSpellCharges = function(id) return nil end, GetSpellDisplayCount = function(id) return nil end }
Spells.InvalidateCache()
